	...
	this.run = function (specificFiles) {
		...
		if (specificFiles) {
			var exclusiveFiles = specificFiles.filter(function (file) {
				return this.exclusiveTests.indexOf(file) !== -1;
			}, this);

			if (exclusiveFiles.length === this.exclusiveTests.length) {
				promise = api.run(specificFiles);
			} else {
				promise = api.run(specificFiles, true);
			}
		} else {
			promise = api.run(files);
		}

		this.busy = promise.then(function () {
			logger.finish();
		}, rethrowAsync);
	};
...
Watcher.prototype.trackExclusivity = function (api) {
	...
	api.on('stats', function (stats) {
		self.updateExlusivity(stats.file, stats.hasExclusive);
	});
};

Watcher.prototype.updateExlusivity = function (file, hasExclusive) {
	var ix = this.exclusiveTests.indexOf(file);

	if (hasExclusive && ix === -1) {
		this.exclusiveTests.push(file);
	} else if (!hasExclusive && ix !== -1) {
		this.exclusiveTests.splice(ix, 1);
	}
};
...
Watcher.prototype.cleanUnlinkedTests = function (unlinkedTests) {
	unlinkedTests.forEach(function (testFile) {
		...
		this.updateExlusivity(testFile, false);
	}, this);
};
